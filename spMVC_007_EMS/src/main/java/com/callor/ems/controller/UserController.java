...
public class UserController {
	...
	/*
	 * UserVO 를 ModelAttrivute 로 생성하여 join.jsp 에게 보내기
	...
	public String join(@Valid UserVO userVO, BindingResult result) {
		...
		// 이메일 형식 유효성 검사를 통과하지 못하면 다시 join.jsp 에게
		// 되돌려 보낸다.
		if(result.hasErrors()) {
			return null;
		}
	 ...
	 * 데이터가 있으면 그대로 통과를 하고,
	 * 만약 데이터가 없어면 SessionAttribute 에 보관중인 userVO 데이터를 가져와서
	 * 매개변수 userVO 에 세팅한다.
	 */
	...
	public String join_next(
		...
		if(userVO.getKey().equals(userVO.getKey_ok())) {
			 ...
			 * 모두 사용하였으면 데이터를 소멸시켜줘야 한다.
			 */
			sessionStatus.setComplete();
			return "redirect:/join_ok";
		} else {
			log.debug("인증키가 다름");
			return "redirect:/join_next?error=key";
		}
