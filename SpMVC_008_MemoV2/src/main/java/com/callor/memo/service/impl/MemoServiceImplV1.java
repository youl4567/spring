...
public class MemoServiceImplV1 implements MemoService {
	...
	public int insertAndUpdate(MemoDTO memo, MultipartFile file) {
		...
		// insert(새로운 추가)가 되는 경우는 m_seq 가 0 일 것이고
		// Update(수정) 가 되는 경우는 m_seq 가 0이 아닐 것이다
		...
		if(m_seq !=0) {
			...
			// 저장된 파일 이름과 새롭게 업로드 된 파일 이름이 다르면
			if(fileName.equals(memo.getM_image())); {
	...
	public int delete(Long seq) {
		...
		// 파일 이름을 filedelete() 에게 보내서 파일을 삭제
