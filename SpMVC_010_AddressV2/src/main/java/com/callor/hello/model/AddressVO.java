...
/*
 * 자바에서 데이터를 담아서 메서드와 메서드 사이에 전달하기 위한 장바구니
 * 
 * model 클래스, domain 클래스, VO 클래스, DTO 클래스
 * 
 * method 에 전달해야할 매개변수(parameter, argument) 가 2개 초과될 때는
 * 개별 변수로 선언하지 않고, VO(DTO) 클래스를 선언하여 사용하는 것이
 * 가독성이나 편리성에서 유리하다.
 * 
 * VO 클래스는 데이터 구조
 ...
 * 
 * DB 와 연동을 하고 JSP 와 view 와 연결할 때
 * 단일 단어로 되어있는 변수를 사용하는 경우
 * 엉뚱한 데이터가 담기거나, 알 수 없는 오류가 발생한다.
 * 
 * 때문에 가급적 snacke case 변수명을 선언하는 것이 좋다.
 */
@Getter
@Setter
@AllArgsConstructor
