document.addEventListener("DOMContentLoaded", () => {
  ...
  // 화면이 모두 그려지면 username input box 에 포커싱하기
  username?.focus();
  ...
  */
  username?.addEventListener("blur", (e) => {
    ...
    fetch(`${rootPath}/user/idcheck/${current.value}`)
      .then((res) => res.text())
      .then((result) => {
        // 서버에서 OK를 보내면 체크한 Username 은 사용해도 된다 라는
        if (result === "OK") {
  ...
  re_password?.addEventListener("input", (e) => {
    ...
    if (current.value === password?.value) {
      ok_msg(error, " * 비밀번호 확인이 일치합니다. 잘했어요");
    } else {
      error_msg(error, "* 비밀번호 확인이 일치하지 않습니다.");
    }
  }); // end re_password
    ...
    fetch(`${rootPath}/user/emailcheck?email=${current.value}`)
      ...
      .then((result) => {
        if (result === "OK") {
          ...
          error.innerText = " * 사용할 수 있는 email 입니다!";
        } else {
          error.classList.remove("w3-text-blue");
          error.classList.add("w3-text-red");
          error.innerText = " * 이미 등록된 email 입니다. 다시 작성해주세요!!";
          current.foucs();
        }
      });
  });
